...
La partición de datos tendrá tres niveles. El entrenamiento usa la distribución propia de cada surrogate. La validación usa una muestra independiente uniforme de 50k puntos por familia de modelo y sirve para monitorizar convergencia, activar el scheduler y seleccionar el checkpoint mediante una regla fijada antes del experimento. Esta selección no cambia el presupuesto de entrenamiento: no hacemos early stopping, solo conservamos el mejor estado observado bajo la misma métrica para todos los surrogates comparables.
