...
El resto del documento se organiza como sigue. La \autoref{sec:revision} sintetiza la revisión bibliográfica que ha guiado las decisiones metodológicas. La \autoref{sec:metodologia} fija los casos de estudio y la metodología experimental: targets, normalización, solvers, dominio, muestreo y evaluación por bins. La \autoref{sec:experimentos} describe los cinco experimentos y los once surrogates que los sostienen. La \autoref{sec:resultados} reporta los resultados de cada experimento. La \autoref{sec:discusion} discute alcance, limitaciones y trabajo en curso.
...
\paragraph{Constantes del pipeline.}
La configuración de la red, el optimizador y el régimen de entrenamiento son comunes a todos los surrogates. Se fijan en código y no se mueven salvo que un hallazgo experimental lo justifique; los valores se recogen en la \autoref{tab:constantes}.
...
\bottomrule
\end{tabular}
\caption{Constantes del pipeline comunes a todos los surrogates.}
...
\bottomrule
\end{tabular}
\caption{Dominio contractual común y dominio adicional de Heston. Es deliberadamente amplio: incluye weeklies, wings profundas y volatilidades de estrés.}
\label{tab:dominio}
\end{table}
...
\end{tikzpicture}
\caption{Partición $5 \times 5$ del dominio en bins de moneyness y vencimiento. Los bins ATM combinados con weekly, short y medium-short (sombreados en azul intenso) son los críticos para E3 por su mayor curvatura.}
\label{fig:bins}
\end{figure}
...
Cinco experimentos comparativos sostienen el trabajo, uno por cada dimensión del problema que aislamos para estudiarla. Cada experimento responde a una pregunta única, varía una sola dimensión y mantiene todo lo demás fijo. Esta rigidez es la que permite atribuir cualquier diferencia observada a la dimensión que estamos variando y no a otras variables que se nos hayan colado por accidente: si se cambia activación no se cambia dataset; si se cambia sampler no se cambia arquitectura; si se cambia pérdida se conservan tamaño, muestra y protocolo de entrenamiento. Las semillas de generación y entrenamiento quedan fijadas por configuración y registradas para asegurar reproducibilidad, no elegidas después de observar resultados. Para los experimentos con clasificación operativa, los umbrales positivo fuerte/débil/negativo se fijan antes de ver los datos para evitar interpretaciones oportunistas al redactar conclusiones.
...
\midrule
E1 & Discrepancia $\text{MAE}(C/K)$ vs $\text{MAE}_{\text{IV}}$ por bin & Sin umbral; Vega/proxy, percentiles altos y fallos de IV \\
E2 & $\text{MAE}_\Delta$ por bin & Sin umbral; activaciones suaves vs ReLU y precio como control \\
E3 & $\text{MAE}_{\text{IV}}$ en bins ATM con weekly, short, medium-short & Positivo fuerte si mejora $\geq 10\%$ y global empeora $\leq 10\%$; precio como control \\
E4 & $\text{speedup} = t_{\text{solver}} / t_{\text{surrogate}}$ por tamaño de lote & Lotes $10^2$--$10^5$, 3 warmups + 10 repeticiones, mediana, CPU/GPU separado \\
...
E2 es el experimento más cargado en surrogates porque la activación es la única decisión arquitectónica que afecta directamente la calidad de las derivadas. La intuición se ve en la \autoref{fig:activations}: ReLU, Softplus, Swish y tanh producen perfiles de salida parecidos, pero solo ReLU tiene una derivada discontinua en cero, mientras que las otras tres son suaves en todo el dominio. Cuando la Delta del surrogate se obtiene por autograd, esa discontinuidad de la activación se propaga al gradiente y degrada la calidad de la Delta aprendida, aunque el error en precio sea comparable. Entrenar las cuatro activaciones sobre Black-Scholes nos sirve como control en un entorno donde conocemos la solución exacta y las Deltas analíticas, y entrenarlas sobre Heston confirma o desmiente el patrón en el caso real.
...
E2 compara las cuatro activaciones (ReLU, Softplus, Swish y tanh) sobre Black-Scholes (BS-1..4) y sobre Heston (H-1..4), manteniendo arquitectura, optimizador, dataset y protocolo de evaluación fijos. Las semillas de entrenamiento están fijadas por configuración y registradas en los checkpoints, por lo que el experimento es reproducible aunque no use una semilla común para todas las activaciones. La métrica primaria es $\text{MAE}_\Delta$ por bin; $\text{MAE}(C/K)$ se reporta como control para verificar que las activaciones suaves no degradan el nivel del precio. La hipótesis es que la falta de suavidad de ReLU se propaga a la Delta calculada por autograd, generando errores sustancialmente mayores aunque el precio sea comparable.
...
E3 contrasta H-3 (muestreo uniforme) y H-5 (muestreo enfocado en $m \in [0.7, 1.3]$ y vencimientos cortos) evaluados sobre el mismo test balanceado, manteniendo todo lo demás fijo. La métrica primaria es $\text{MAE}_{\text{IV}}$ en los bins ATM combinados con weekly, short y medium-short. Se aplica la clasificación operativa fijada en la \autoref{tab:metricas}: positivo fuerte si la mejora local supera el 10\,\% sin que el error global empeore más del 10\,\%; positivo débil si la mejora es menor o el deterioro global es moderado; negativo si no mejora los bins críticos o el coste global es excesivo.
