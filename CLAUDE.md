...
- `docs/experimento-escalado-datos.md` — el 50x posterior: el plateau de `MAE(C/K)~1e-2` era data-bound; con 50x los 11 surrogates bajaron entre 37% y 91%.
...
- Pipeline 50x (10M BS / 25M Heston): `python scripts/generate_all_50x_progress.py`. En la documentación tardó ~8h20 en CPU con `--parallel 32` (i9 14900K) y 5.16 GB de disco; rejection rate global 0.0186%.
...
- `python scripts/train_all_parallel_50x.py --parallel 2` — pipeline 50x. `--parallel 2` es el techo en una RTX 4060 8 GB porque cada proceso Heston-25M con `--preload-to-device` consume ~1.7 GB de VRAM.
...
H-3 es el surrogate central (aparece en los cinco experimentos). Semillas: 101-104 para BS, 201-207 para Heston. Las mismas semillas se reutilizan en el experimento 50x para que cualquier diferencia sea atribuible al volumen de datos, no a un sorteo distinto.
...
- Particiones: train (sampler del surrogate), validación uniforme 50k (2.5M en 50x), test balanced 5k/bin → 125k (250k/bin → 6.25M en 50x). Test balanceado **compartido** por familia: H-3 y H-5 se evalúan sobre el mismo test, aunque H-5 haya entrenado focused — eso es lo que permite ver dónde gana y dónde pierde el sampler enfocado.
...
- Arquitectura por defecto: MLP 4 capas × 128 unidades (~50k parámetros), Swish, Adam `lr=1e-3` con reduce-on-plateau, batch 1024 baseline (32768 en orquestadores 50x), 100 épocas máximo.
...
- `src/evaluation/` y `src/experiments/` — planificados en `architecture.md` pero **sin código todavía**. La idea es `BinPartition` + `BinEvaluator(partition, OptionPricer)` con inyección de dependencias, y cinco `Experiment` subclases (`PriceVsIVStudy`, `ActivationStudy`, `SamplingStudy`, `EfficiencyStudy`, `DMLStudy`) todas con `.run() -> Report`.
...
- **Fases 2-4** (BS, Heston, eficiencia): aún abiertas en el plan original. El experimento 50x posterior demostró que los baselines eran data-bound: tras escalar datasets ×50 los 11 surrogates bajaron entre 37% y 91% en `MAE(C/K)` de validación, así que la conversación arquitectónica del paper de Chen (escalar capas/anchura) queda diferida hasta que el régimen 50x se asiente.
...
- **No commitees `data/`** (gitignored) ni cambies rangos del dominio, semillas o nombres de archivo sin documentarlo: rompería la comparabilidad de cualquier resultado nuevo con los baselines existentes y con el experimento 50x.
