"""Parallel training orchestrator for the 50x dataset pipeline.

Same dispatch logic as ``train_all_parallel.py`` but pointing to the
``_10M_/_25M_/_5M_/_2500k_`` datasets generated by
``generate_all_50x_progress.py``.

Default ``--parallel 2`` instead of 4 because Heston-25M with
``--preload-to-device`` holds ~1.7 GB on the GPU per process; on an RTX 4060
(8 GB total, ~6.3 GB free after Windows) 3+ concurrent Heston-25M trainings
would exhaust VRAM. BS-only or Heston-5M phases tolerate higher parallelism
...
SURROGATES: list[SurrogateSpec] = [
    ...
    SurrogateSpec("H-3-small", "heston_train_5M_uniform.npz",          "heston_validation_2500k_uniform.npz",      "price",        "swish",    16384, 206),
...
def build_command(spec: SurrogateSpec, args: argparse.Namespace) -> list[str]:
    cmd = [
        sys.executable,
        str(TRAIN_SCRIPT),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train all 11 surrogates in parallel on a single GPU (50x datasets).")
    parser.add_argument("--parallel", type=int, default=2, help="Number of concurrent training subprocesses (default: 2 to fit Heston-25M with preload).")
    ...
    print(
        f"[parallel] data_dir={args.data_dir.name}/  results_dir={args.results_dir.name}/  logs={args.log_dir.relative_to(REPO_ROOT)}",
        flush=True,
    )
